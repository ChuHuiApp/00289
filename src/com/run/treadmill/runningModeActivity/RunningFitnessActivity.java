...
import com.run.treadmill.runningParamManager.RunningParamFitness;
...
public class RunningFitnessActivity extends BaseActivity implements OnClickListener,
	TimerCallBack, ParamRefreshTimerCallBack, HrBeatTimerCallBack  {

	private String TAG = "RunningFitnessActivity";
	...
	private RelativeLayout warmup_page;
	private ImageView btn_sportmode_warmup;
    	...
    	if( !UserInfoManager.getInstance().mIsMediaMode && needResume ) {
    		...
    		if( UserInfoManager.getInstance().sysRunStatus == SYSRUNSTATUS.STA_ERROR_PAGE.ordinal() ) {
    			...
    			if( mRunningParam.HRC_RUN_STATUS == CTConstant.HRC_NO_HR_STATUS || 
    					mRunningParam.HRC_RUN_STATUS == CTConstant.FITNESS_OVER_TAG_HR ) {
    				/*startTimerToRefreshParam();*/
    				startDelayTimerToRefreshParam();
    			}
    ...
    public void reSettingParamShow() {
		...
		if( !mRunningParam.isCoolDownPage  ) {
			...
			Log.d(TAG,"mRunningParam.mShowRunTime===hahah== : " + mRunningParam.mShowRunTime + 
	...
	public void initParamDataAndRunTimer() {
		mRunningParam = new RunningParamFitness(mContext);
		...
		if( UserInfoManager.getInstance().getRunMode() == 
			...
			if ( UserInfoManager.getInstance().getSex() == CTConstant.GENDER_BOY ) {
				System.arraycopy( mRunningParam.proMaleLevelTable, 
						0, mRunningParam.mLevelItemValueArray,
						0, InitParam.TOTAL_RUN_INTERVAL_NUM );
			} else {
				System.arraycopy( mRunningParam.proFemaleLevelTable, 
						0, mRunningParam.mLevelItemValueArray,
						0, InitParam.TOTAL_RUN_INTERVAL_NUM );
			}

		}
//		Log.d(TAG, "mLevelItemValueArray === "+mRunningParamManager.mLevelItemValueArray[0]);
		...
		if( UserInfoManager.getInstance().getRunMode() == 
				RunMode.IDX_HOME_FITNESS_MODE.ordinal() ) {
			UserInfoManager.getInstance().curLevel = mRunningParam.warmUpLevel;
			text_level.setText( mRunningParam.warmUpLevel + "" );
			if( !isMetric )
				text_speed.setText( MyUtils.getSpeedMileOneP(UserInfoManager.getInstance().rpm) + " mph" );
			else
				text_speed.setText( MyUtils.getSpeedKmOneP(UserInfoManager.getInstance().rpm) + " kph" );
	...
	private void startDelayTimerToRefreshParam() {
		mRunParamRefreshTimer.closeTimer();
		mRunningParam.isRefreshPage = true;
		mRunParamRefreshTimer.startParamRefreshTimer(600, RUN_VALUE_GET_TIME, this);
		startTimerToHrBeat();
	}
	...
	public void onClick(View arg0) {
		switch (arg0.getId()) {
			...
			case R.id.btn_sportmode_down:
				...
				Log.d(TAG,"btn_inclinemode_down");
			...
			case R.id.btn_sportmode_media_in:
				...
				min_Chart_View.clearAnimation();
				if ( min_Chart_View.getVisibility() == View.VISIBLE ) {
...
//				Log.d(TAG, "=====2=====mRunningParam.startPauseTime =" + mRunningParam.startPauseTime);
			...
			case R.id.btn_pause_quit:
				...
				stopRunAndSkipToSumActivity(mRunningParam.alreadyRunTime, 
						mRunningParam.alreadyRunDistance, mRunningParam.alreadyRunCalories, true, false);
				break;
			...
			case R.id.btn_sportmode_cooldown:
				...
				stopRunAndSkipToSumActivity(mRunningParam.alreadyRunTime, 
						mRunningParam.alreadyRunDistance, mRunningParam.alreadyRunCalories, true, true);
				break;
			case R.id.btn_sportmode_warmup:
				if ( isNeedBuzzer == false ) {
					isNeedBuzzer = true;
				} else {
					Support.buzzerRingOnce();
				}
				Log.d(TAG," mRunningParam.mShowRunDistance  " + mRunningParam.mShowRunDistance
						+ "mRunningParam.mShowRunCalories " + mRunningParam.mShowRunCalories);
				mRunningParam.mShowRunDistance = 0;
				mRunningParam.mShowRunCalories = 0;
				text_calories.setText( 0.0 + " kcal");
				if( !isMetric )
				...
				enbleOnclickEvent();
				mBlurringView.setVisibility(View.GONE);
		        warmup_page.setVisibility(View.GONE);
				btn_sportmode_stop.setVisibility(View.VISIBLE);
				top_layout.setVisibility(View.VISIBLE);

				mRunningParam.isWarmUpPage = false;
				mWarmUpPageTimer.closeTimer();

				initRunParamAndStartRun(true, false);
				break;
			default:
				break;
		}
			...
			switch (msg.what) {
				case MEG_COUNT_DOWN:
					...
					if (count == -1) {
						...
						startTimerToRefreshParam();
						if( UserInfoManager.getInstance().getRunMode() == 
								RunMode.IDX_HOME_FITNESS_MODE.ordinal() && !(Boolean) msg.obj ) {
							enterWarmUpPage();
						} else {
				...
				case MSG_REFRESH_PARAM_VALUE:
					...
					if( mRunningParam.isWarmUpPage ) {
						warmUpPageRefreshParam();
					} else if( mRunningParam.isCoolDownPage ) {
				...
				case R.id.btn_sportmode_warmup:
					enbleOnclickEvent();
					btn_sportmode_warmup.performClick();
					break;
...
//		Log.d("runDistance"," ==0===== " + mRunningParam.alreadyRunDistance + " timeValue " + mRunningParam.alreadyRunTime);
		...
		String runCaloriesStr = MyUtils.getShowIntValue(showRunCalories);
		/*Log.d("runCaloriesStr"," ==0===== " + runCaloriesStr);*/
	...
	public void stopRunAndSkipToSumActivity( float returnRunTime, float returnRunDistance ,
			float runCalories , boolean isShowSummary, boolean isCoolDownEnter ) {
		...
		intent.putExtra(CTConstant.IS_SHOW_SUMMARY, isShowSummary);
	...
	public void showErrorMsgPage() {
		if( mRunningParam.HRC_RUN_STATUS == CTConstant.HRC_NO_HR_STATUS ) {
			...
			stopRunAndSkipToSumActivity(mRunningParam.alreadyRunTime, 
					mRunningParam.alreadyRunDistance, mRunningParam.alreadyRunCalories, false, false);
		} else if( mRunningParam.HRC_RUN_STATUS == CTConstant.FITNESS_OVER_TAG_HR ) {
			enterCoolDownPage();
			mRunningParam.coolDownLevel = 
					mRunningParam.mLevelItemValueArray[mRunningParam.runStageNum];
		} else if( mRunningParam.HRC_RUN_STATUS == CTConstant.LOW_RPM ) {
			disableOnclickEvent();
			/*topLayotIsFrontInBlurringView(true);*/
			Toast toast = Toast.makeText(mContext, "INVALID TEST - LOW RPM", Toast.LENGTH_SHORT);                    
	        toast.show();
		} else if( mRunningParam.HRC_RUN_STATUS == CTConstant.OUT_RANGE_RPM ) {
			disableOnclickEvent();
			/*topLayotIsFrontInBlurringView(true);*/
			Toast toast = Toast.makeText(mContext, "INVALID TEST - RPM OUT OF RANGE", Toast.LENGTH_SHORT);                    
	        toast.show();
		} else {
		...
		if ( mRunningParam.HRC_RUN_STATUS == CTConstant.NO_RPM_STATUS ) {
			...
			error_status_layout.setVisibility(View.VISIBLE);
			error_status_icon.setImageResource(R.drawable.img_pop_norpm);
		}
	...
	public void callback() {
		...
		if ( mRunningParam.isWarmUpPage ) {
			mRunningParam.isWarmUpPage = false;
			mWarmUpPageTimer.closeTimer();
			mMsgHandler.sendEmptyMessage(R.id.btn_sportmode_warmup);
//			btn_sportmode_warmup.performClick();
		}
	...
	private void enterWarmUpPage()
	{
        ...
        warmup_page.setVisibility(View.VISIBLE);
		UserInfoManager.getInstance().curLevel = mRunningParam.warmUpLevel;
		text_level.setText( mRunningParam.warmUpLevel + "" );
		if( !isMetric )
		text_speed.setText( MyUtils.getSpeedMileOneP(UserInfoManager.getInstance().rpm) + " mph" );
...
//		mWarmUpPageTimer = new BaseTimer();
		mWarmUpPageTimer.startTimer( PAUSE_PAGE_TIMER_DELAY, this);
		mRunningParam.isWarmUpPage = true;
	...
	public void initSurfaceView() {
		...
		warmup_page = (RelativeLayout) findViewById(R.id.warmup_page);
		btn_sportmode_warmup = (ImageView) findViewById(R.id.btn_sportmode_warmup);
	...
	public void setListenerEvent() {
		...
		btn_sportmode_warmup.setOnClickListener(this);
...
//    	super.onBackPressed();
	...
	public void warmUpPageRefreshParam() {
		/*Log.d(TAG,"warmUpPageRefreshParam");*/
		mRunningParam.getRunStageNum();
		...
		float curRunCalories = mInterfaceUtils.getRunCaloriesPerSec(
				UserInfoManager.getInstance().getUserInfo(), MyUtils.getSpeedKmOneP(UserInfoManager.getInstance().rpm), 
				UserInfoManager.getInstance().curLevel);
		/*mRunningParam.alreadyRunCalories += curRunCalories;*/
		mRunningParam.mShowRunCalories += curRunCalories;
		String runCaloriesStr = MyUtils.getShowIntValue(mRunningParam.mShowRunCalories);

		if( UserInfoManager.getInstance().getTargetTime(UserInfoManager.getInstance().getRunMode()) != 0f ) {
			float tempTime = UserInfoManager.getInstance().getTargetTime(UserInfoManager.getInstance().getRunMode()) * 60 * 1000 -
					mRunningParam.warmUpRunTime;
			text_time.setText( MyUtils.getMsToSecTimeValue(tempTime) + "" );
		} else {
			text_time.setText( MyUtils.getMsToSecTimeValue(mRunningParam.warmUpRunTime) + "" );
		}
		...
		text_calories.setText( runCaloriesStr + " kcal");
		text_pulse.setText(UserInfoManager.getInstance().ecgValue + "");
		text_watt.setText( mInterfaceUtils.getWattValue(UserInfoManager.getInstance().rpm, 
				UserInfoManager.getInstance().curLevel) +"");
		if( !isMetric )
			text_distance.setText(MyUtils.getDistanceMileValue(mContext, showRunDistance) + "mile");
		else
		...
		/*if( mRunningParam.isAccOneMin() ) {
			Log.d(TAG,"isAccOneMin set speed warmUp list " + mRunningParam.alreadyRunTime);
			UserInfoManager.getInstance().addLevelList(UserInfoManager.getInstance().curLevel, 
					mRunningParam.alreadyRunTime);
			UserInfoManager.getInstance().addHrList(UserInfoManager.getInstance().ecgValue, 
					mRunningParam.alreadyRunTime);
	...
	public void onCmdEvent(Message msg) {
		...
		switch (msg.arg1) {
			case Command.KEY_CMD_QUICK_START:
				if ( mRunningParam.isWarmUpPage == true ) {
					Log.d(TAG,"ENTER R.id.btn_sportmode_warmup");
					Support.keyToneOnce();
					isNeedBuzzer = false;
					mMsgHandler.sendEmptyMessage(R.id.btn_sportmode_warmup);
					break ;
			...
			case Command.KEY_CMD_STOP_CANCEL:
				...
				if( mRunningParam.isPausePage == true ) {
					...
					isNeedBuzzer = false;
					mMsgHandler.sendEmptyMessage(R.id.btn_pause_quit);
				} else if ( mRunningParam.isWarmUpPage == true ) {
					/*mRunningParam.isWarmUpPage = false;
					mWarmUpPageTimer.closeTimer();
					Support.keyToneOnce();
					isNeedBuzzer = false;
	...
	private void releaseQuitProc() {
		...
		MyUtils.animLeftFinishActivity(RunningFitnessActivity.this, intent);
	}
	...
	public void enbleOnclickEvent() {
		...
		/*btn_inclinemode_roller.setEnabled(true);
		btn_speedmode_roller.setEnabled(true);
		btn_inclinemode_up.setEnabled(true);
		btn_inclinemode_down.setEnabled(true);
	...
	public void disableOnclickEvent() {
		...
		/*btn_inclinemode_roller.setEnabled(false);
		btn_speedmode_roller.setEnabled(false);
		btn_inclinemode_up.setEnabled(false);
		btn_inclinemode_down.setEnabled(false);
		btn_sportmode_up.setEnabled(false);
		btn_sportmode_down.setEnabled(false);*/
	...
	public boolean dispatchTouchEvent(MotionEvent event) {
    	...
    	msg.arg1 = Command.KEY_CMD_QUICK_START;
    	this.onCmdEvent(msg);
