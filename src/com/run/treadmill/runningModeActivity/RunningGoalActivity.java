...
import com.run.treadmill.runningParamManager.RunningParamGoal;
...
public class RunningGoalActivity extends BaseActivity implements OnClickListener, 
	TimerCallBack, ParamRefreshTimerCallBack, HrBeatTimerCallBack  {

	private String TAG = "RunningGoalActivity";
	...
	private RunningParamGoal mRunningParam = null;
    ...
    public void reSettingParamShow() {
		...
		if( mRunningParam.isSetDistance ) {
			...
			if( showRunDistance < 0 ) {
				showRunDistance = 0.0f;
			}
		}
	...
	public void initParamDataAndRunTimer() {
		mRunningParam = new RunningParamGoal(mContext);
...
//		Log.d(TAG, "mSpeedItemValueArray === "+mRunningParamManager.mSpeedItemValueArray[0]);
		...
		if( UserInfoManager.getInstance().getRunMode() == 
			...
			if( UserInfoManager.getInstance().getTargetRunType() == 
					CTConstant.RUN_REFERENCE_BY_TIME ){
			...
			} else if ( UserInfoManager.getInstance().getTargetRunType() == 
					CTConstant.RUN_REFERENCE_BY_DISTANCE ){
				mRunningParam.isSetDistance = true;
				text_time.setText( "00:00" );
				if( !isMetric )
					text_distance.setText(MyUtils.getDistanceMileValue(mContext, UserInfoManager.getInstance().getTargetDistance()) + "mile");
				else
				text_distance.setText( UserInfoManager.getInstance().getTargetDistance() + " km" );
			} else if ( UserInfoManager.getInstance().getTargetRunType() == 
					CTConstant.RUN_REFERENCE_BY_CALORIES ){
				mRunningParam.isSetCalories = true;
				text_time.setText( "00:00" );
				text_calories.setText( (int)UserInfoManager.getInstance().getTargetCalorie() + " kcal" );
			}
...
//		Log.d("runDistance"," ==0===== " + mRunningParam.alreadyRunDistance + " timeValue " + mRunningParam.alreadyRunTime);
	...
	public void stopRunAndSkipToSumActivity( float returnRunTime, float returnRunDistance ,
		...
		if ( mRunningParam.isRunEnd() ) {
			if( UserInfoManager.getInstance().getTargetRunType() == 
					CTConstant.RUN_REFERENCE_BY_TIME ) {
				UserInfoManager.getInstance().setTotalTime(
						mRunningParam.runMaxTime * 60 * 1000);
			} else if ( UserInfoManager.getInstance().getTargetRunType() == 
					CTConstant.RUN_REFERENCE_BY_DISTANCE ) {
				UserInfoManager.getInstance().setTotalDistance(
						UserInfoManager.getInstance().getTargetDistance());
			} else if ( UserInfoManager.getInstance().getTargetRunType() == 
					CTConstant.RUN_REFERENCE_BY_CALORIES ) {
				UserInfoManager.getInstance().setTotalCalories(
						UserInfoManager.getInstance().getTargetCalorie());
			}
		}
...
//    	super.onBackPressed();
	...
	public void coolDownPageRefreshParam() {
		/*mRunningParam.alreadyRunTime = mInterfaceUtils.getRunTime(mContext);

		mRunningParam.getRunStageNum();

		float curRunDistance = 
				mInterfaceUtils.getRunMileage(mContext, 
						UserInfoManager.getInstance().curSpeed, (float) (1/60.0/60.0));
		mRunningParam.alreadyRunDistance += curRunDistance;

		float curRunCalories = mInterfaceUtils.getRunCaloriesPerSecond(
				UserInfoManager.getInstance().getUserInfo(), 
				UserInfoManager.getInstance().curSpeed, 
				UserInfoManager.getInstance().curIncline);
		mRunningParam.alreadyRunCalories += curRunCalories;
		String runCaloriesStr = MyUtils.getShowIntValue(mRunningParam.alreadyRunCalories);*/
	...
	private void releaseQuitProc() {
		...
		MyUtils.animLeftFinishActivity(RunningGoalActivity.this, intent);
	}
