...
import com.run.treadmill.runningParamManager.RunningParamVR;
...
import com.run.treadmill.videoControl.VideoPlayerSelf;
...
public class RunningVRActivity extends BaseActivity implements OnClickListener, 
	TimerCallBack, ParamRefreshTimerCallBack, HrBeatTimerCallBack {

	private String TAG = "RunningVRActivity";
	...
	private RunningParamVR mRunningParam = null;
	...
	private LinearLayout videoPlayer_layout;
	private VideoPlayerSelf mVideoPlayerSelf;

	private String videoSrc = "/storage/card" + "/Austria_Hallstatt.mp4";
//	private final String videoSrc = "/mnt/sdcard/DCIM" + "/00.mp4";
	...
	public void initParamDataAndRunTimer() {
		mRunningParam = new RunningParamVR(mContext);
		if( UserInfoManager.getInstance().getRunMode() == 
			...
			/*for( int i = 0 ; i < 30; i++ ) {
				Log.d(TAG, "mInclineItemValueArray " + mRunningParam.mInclineItemValueArray[i] + "  " + UserInfoManager.getInstance().vrSceneVideoNo);
			}*/
		} 
...
//			String outerSdCardRootPath = Environment.getExternalStorageDirectory().getAbsolutePath();
//			Log.d(TAG,"outerSdCardRootPath = " + outerSdCardRootPath);
			videoSrc = CTConstant.vrVideoPath[UserInfoManager.getInstance().vrSceneVideoNo];
//			videoSrc = "/mnt/sdcard/DCIM/222.mp4";
			Log.d(TAG,"isCheckExist   ===" +  videoSrc);
			if( Support.isCheckExist(videoSrc) ) {
				Log.d(TAG,"isCheckExist   ==============");
				videoPlayer_layout.setVisibility(View.VISIBLE);
				mVideoPlayerSelf = new VideoPlayerSelf(mContext, R.id.videoPlayerSurface, 
						videoPlayer_layout, videoSrc);
...
//					Log.d(TAG, "=====2=====mRunningParam.startPauseTime =" + mRunningParam.startPauseTime);
					...
					enterPausePage();
					break;
				case CTConstant.MSG_VR_FRESH_TIME:
					int[] incValues = (int[]) msg.obj;
					mRunningParam.setInclinePos(incValues[0]);
					break;
				case R.id.btn_pause_continue:
...
//		Log.d("runDistance"," ==0===== " + mRunningParam.alreadyRunDistance + " timeValue " + mRunningParam.alreadyRunTime);
	...
	public void initSurfaceView() {
		...
		layout_rpm_view.setVisibility(View.GONE);
		...
		videoPlayer_layout = (LinearLayout) findViewById(R.id.videoPlayer_layout);
	}
	...
	public void setLevelValue(int isUp, int mLevel) {
		...
		if( StorageParam.getRunMode(mContext) == 
				...
				for( int i = 0; i < InitParam.TOTAL_RUN_INTERVAL_NUM; i++ ) {
...
//    	super.onBackPressed();
	...
	public synchronized void videoPlayerPlayer() {
	...
	public void setVideoSpeed( int videoPlayerSpeed ) {
		if( mVideoPlayerSelf != null ) {
			mVideoPlayerSelf.setSpeedCtrl(videoPlayerSpeed);
//			mRunningParam.setCommonCount( (int)Math.ceil(50.0f / videoPlayerSpeed) );
		}
	}

	public void onVideoPlayerDestory() {
		if( mVideoPlayerSelf != null ) {
	...
	public void showSpeedProfile() {
		min_Chart_View.clearAnimation();
		if ( !isMetric ) {
			min_Chart_View_Unit.setImageResource(R.drawable.img_sportmode_profile_speed_calibration_mile_1);
		} else {
			min_Chart_View_Unit.setImageResource(R.drawable.img_sportmode_profile_speed_calibration_km_1);
		}
	...
	public void showInclineProfile() {
		min_Chart_View.clearAnimation();
		min_Chart_View_Unit.setImageResource(R.drawable.img_sportmode_profile_incline_calibration_1);
		min_Chart_View_bg.setImageResource(R.drawable.img_sportmode_profile_incline_1);
		
	}

	private synchronized void releaseQuitProc() {
		...
		MyUtils.animLeftFinishActivity(RunningVRActivity.this, intent);
	}
