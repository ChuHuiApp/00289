...
import com.run.treadmill.runningParamManager.RunningParamQuick;
...
public class RunningQuickActivity extends BaseActivity implements OnClickListener, 
	TimerCallBack, ParamRefreshTimerCallBack, HrBeatTimerCallBack {

	private String TAG = "RunningQuickActivity";
	...
	private RunningParamQuick mRunningParam = null;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if( UserInfoManager.getInstance().mIsMediaMode ) {
			...
			if ( !language.endsWith("zh") ) {
				if ( UserInfoManager.getInstance().mediaMode == 
						MediaENItem.ITEM_MEDIA_FACEBOOK.ordinal() ) {
					...
					Support.doStartApplicationWithPackageName(mContext,
							CTConstant.mediaENPkgName[UserInfoManager.getInstance().mediaMode]);
				}
				
			} else {
				Support.doStartApplicationWithPackageName(mContext,
						CTConstant.mediaPkgName[UserInfoManager.getInstance().mediaMode]);
			}

			FloatWindowManager.getInstance(mContext).runningActivityStartMedia(mRunningParam);
    ...
    public void onResume() {
    	...
    	if ( isEnterPause && 
    		...
    		isEnterPause = false;
    		if ( FloatWindowManager.getInstance(mContext).quitBackHomeOrRunningSurface() ) {
    			finish();
    		}
    	}
    ...
    public void reSettingParamShow() {
		...
		Log.d("runDistance"," ==1===== " + mRunningParam.alreadyRunDistance + " timeValue " + mRunningParam.alreadyRunTime);
	...
	public void initParamDataAndRunTimer() {
		mRunningParam = new RunningParamQuick(mContext);
...
//		mMsgHandler.remeCallbacksAndMessages(null);
	...
	public void runParamProc() {
		...
		if ( mRunningParam.isJumpInterface ) {
			return ;
		}
		showErrorMsgPage();
...
//		Log.d("runDistance"," ==0===== " + mRunningParam.alreadyRunDistance + " timeValue " + mRunningParam.alreadyRunTime);
	...
	public void showErrorMsgPage() {
		...
		if ( mRunningParam.HRC_RUN_STATUS == CTConstant.NO_RPM_STATUS ) {
			...
			/*stopRunAndSkipToSumActivity(mRunningParam.alreadyRunTime, 
					mRunningParam.alreadyRunDistance, mRunningParam.alreadyRunCalories);
			mRunningParam.isJumpInterface = true;*/
		}
	}
...
//    	super.onBackPressed();
	...
	private void releaseQuitProc() {
		...
		MyUtils.animLeftFinishActivity(RunningQuickActivity.this, intent);
	}
