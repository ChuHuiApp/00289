...
//进入分享界面后悬浮窗口类
public class OtherBackFloatWindow implements IEventProcess {
	...
	public void startFloat(FloatWindowManager floatWindowManager) {
		...
		SerialUtils.getInstance().registerOtherBackFloatWin(this);
		...
		mFloatWindow = CreateFloatWindow(R.layout.other_back_float_window, Gravity.LEFT | Gravity.TOP,
...
//					MyUtils.leftAnimStartActivityForResult(mContext, new Intent(mContext, HomeActivity.class),false);
					mFloatWindowManager.stopOtherBackFloatWindow();
					/*ShellCmdUtils.getInstance().execCommand("kill " + Support.findPid(mContext, 
							"com.android.settings"));*/
					Support.doStartApplicationWithPackageName(mContext, "com.run.treadmill", 
							"com.run.treadmill.settingActivity.SettingActivity");
	...
	public void stopFloat() {
		...
		if(mFloatWindow != null && mFloatAdded) {
			RemoveView(mFloatWindow);
			SerialUtils.getInstance().unregisterOtherBackFloatWin(this);
			mFloatWindow = null;
		}
	...
	public void errorEventProc(int errorValue) {
		if( errorValue == 1 ) {
			mFloatWindowManager.stopOtherBackFloatWindow();
			Support.killCommonApp(mContext, "com.android.settings");
			/*ShellCmdUtils.getInstance().execCommand("kill " + Support.findPid(mContext, 
					"com.android.settings"));*/
			Support.doStartApplicationWithPackageName(mContext, "com.run.treadmill", 
					"com.run.treadmill.settingActivity.SettingActivity");
